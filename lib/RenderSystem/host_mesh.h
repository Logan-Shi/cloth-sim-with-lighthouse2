/* host_mesh.h - Copyright 2019/2021 Utrecht University
...
public:
	...
	HostMesh(const char* name, const char* dir, const float scale = 1.0f, const bool flatShaded = false, const bool animated = false);
	HostMesh( const tinygltfMesh& gltfMesh, const tinygltfModel& gltfModel, const vector<int>& matIdx, const int materialOverride = -1 );
	~HostMesh();
	// methods
	void LoadGeometry(const char* file, const char* dir, const float scale = 1.0f, const bool flatShaded = false, const bool animated = false);
	void LoadGeometryFromOBJ( const string& fileName, const char* directory, const mat4& transform, const bool flatShaded = false, const bool animated = false);
	...
	bool isAnimated = false;							// true when this mesh has animation data
